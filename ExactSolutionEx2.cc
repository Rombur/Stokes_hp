...
void ExactSolutionEx2 <dim>::vector_value (const Point<dim> &p,
	...
	double 3rd_Deriv_Psi= - (std::pow((1+Lambda),2)) * std::cos(Lambda*Omega)*std::cos((1+Lambda)*std::atan(p(1)/p(0)))- std::pow((1+Lambda),3)*std::sin((1+Lambda)*std::atan(p(1)/p(0)))+ std::pow((1-Lambda),2)*std::cos(Lambda*Omega)*std::cos((1-Lambda)*std::atan(p(1)/p(0)))+ std::pow((1-Lambda),3)*std::sin((1-Lambda)* std::atan(p(1)/p(0)));
	...
	values(2) =  -std::pow(r, (Lambda-1)) * (std::pow((1+Lambda),2)*Deriv_Psi + 3rd_Deriv_Psi )/ (1-Lambda);
		}
...
void ExactSolutionEx2 <dim>::vector_gradient  (const Point<dim> &p,
	...
	gradients[0][0]= 1/(std::pow(r,2)) *(std::pow(r, Lambda)*Lamda*p(0)*( (1+Lambda)*p(1)* ( (std::sin(((1+Lambda)*atan(p(1)/p(0))))* std::cos(Lambda*Omega)) / (1+Lambda) - std::cos((1+Lambda)* std::atan(p(1)/p(0)) ) - ( std::sin((1-Lambda)* std::atan(p(1)/p(0)) )*std::cos(Lambda*Omega) / (1-Lambda) )+ ( std::cos((1-Lambda)*std::atan(p(1)/p(0)))) )  /  p(0)* std::sqrt(1+ std::pow( p(1)/p(0),2))+
			( std::cos(Lambda*Omega)* std::cos((1+Lambda)*std::atan(p(1)/p(0))) + (1+Lambda)*std::sin((1+Lambda)*std::atan(p(1)/p(0)))-  std::cos(Lambda*Omga)*std::cos((1-Lambda)* std::atan(p(1)/p(0))) - (1-Lambda)*std::sin((1-Lambda)*std::atan(p(1)/p(0))) )  /(std::sqrt(1+p(1)/p(0))) ))+
			std::pow(r, Lambda)*
			(
					- ((1+Lambda)*p(1)* ( (std::sin((1+Lambda)*std::atan(p(1)/p(0)))* std::cos(Lambda*Omega) ) /(1+Lambda)  -   std::cos((1+Lambda)*std::atan(p(1)/p(0))) -  (std::sin((1-Lambda)*std::atan(p(1)/p(0))))*std::cos(Lambda*Omega)/(1-Lambda) + std::cos((1-Lambda)*std::atan(p(1)/p(0)))) /(std::pow(p(0),2) * std::sqrt (1+ std::pow(p(1)/p(0),2)))
							...
							+ ( (1+Lambda)*p(1)* (-(std::cos((1+Lambda)*std::atan(p(1)/p(0)))*p(1)* std::cos(Lambda*Omega))/(std::pow(p(0),2)* (1+ std::pow((p(1)/p(0)) ,2))  )   -  (std::sin((1+Lambda)*std::atan(p(1)/p(0)) )* (1+Lambda)*p(1) )/(std::pow(p(0),2)* (1+ std::pow((p(1)/p(0)) ,2))  )  + ( std::cos((1-Lambda)* std::atan(p(1)/p(0)))* p(1)*std::cos(Lambda*Omega))/(std::pow(p(0),2)* (1+ std::pow((p(1)/p(0)) ,2))  )  +
									...
									+ (( std::cos(Lambda*Omega)*(1+Lambda)*p(1)*std::sin((1+Lambda)* std::atan(p(1)/p(0))) ) /(std::pow(p(0),2)*(1+std::pow(p(1)/p(0),2)) )   -   ( std::pow((1+Lambda),2)* p(1)* std::cos((1+Lambda)* std::atan(p(1)/p(0))) )/(std::pow(p(0),2)*(1+std::pow(p(1)/p(0),2)) )   -  ( (1-Lambda)*p(1)*std::cos(Lambda*Omega)*std::sin((1-Lambda)*(std::atan(p(1)/p(0))) ))/(std::pow(p(0),2)*(1+std::pow(p(1)/p(0),2)) )  +
											( std::pow((1-Lambda),2)*p(1)* std::cos((1-Lambda)* std::atan(p(1)/p(0))) )/(std::pow(p(0),2)*(1+std::pow(p(1)/p(0),2)) )  )/(std::sqrt(1+std::pow(p(1)/p(0),2))) ) ;




	gradients[0][1]= 1/(std::pow(r,2))*( (std::pow(r,Lambda))*Lambda*p(1)* ( ( (1+Lambda)*p(1)* (  (std::cos(Lambda*Omega)* std::sin((1+lambda)*std::atan(p(1)/p(0))) )/(1+Lambda) -
			...
			( std::cos(Lambda*Omega)*std::cos((1+Lambda)*std::atan(p(1)/p(0))) + (1+Lambda)*std::sin((1+Lambda)*std::atan(p(1)/p(0)) ) -
					...
					(1-Lambda)*std::sin((1-Lambda)*std::atan(p(1)/p(0))) )/(std::sqrt(1+std::pow(p(1)/p(0) ,2)))  ) ) + std::pow(r,Lambda)* (
							((1+Lambda)* ( (std::sin((1+Lambda)*std::atan(p(1)/p(0)) )*std::cos(Lambda*Omega)  )/(1+Lambda) - std:::cos((1+lambda)*std::atan(p(1)/p(0)))  -
									(std::cos(Lambda*Omega)* std::sin((1-Lambda)* std::atan(p(1)/p(0)) ))/(1-Lambda) +  std::cos((1-Lambda)*std::atan(p(1)/p(0)))  )  )/(p(0)* std::sqrt(1+std::pow(p(1)/p(0) ,2))) -
									( (1+Lambda)*std::pow(p(1),2)* ( (std::cos(Lambda*Omega)* std::sin((1+Lambda)*std::atan(p(1)/p(0))))/(1+lambda) -  std::cos((1+Lambda)*std::atan(p(1)/p(0)))  -(std::cos(Lambda*Omega)* std::sin((1-Lambda)*std::atan(p(1)/p(0))))/(1-Lambda) +
											std::cos((1-Lambda)*std::atan(p(1)/p(0)))  )  )/( std::pow(p(0),3)* std::pow((1+std::pow(p(1)/p(0),2)), 3/2)  ) +
											( (1+Lambda)*p(1)* ( ( std::cos(Lambda*Omega)* std::cos((1+Lambda)*std::atan(p(1)/p(0))) )/(p(0)* (1+std::pow(p(1)/p(0),2))  ) +
													( (1+Lambda)* std::sin((1+Lambda)* std::atan(p(1)/p(0)) ) )/(p(0)* (1+std::pow(p(1)/p(0),2))  ) - ( std::cos(Lambda*Omega)*std::cos((1-Lambda)* std::atan(p(1)/p(0))))/(p(0)* (1+std::pow(p(1)/p(0),2))  ) -
													( std::sin((1-Lambda)*std::atan(p(1)/p(0)))*(1-Lambda) )/(p(0)* (1+std::pow(p(1)/p(0),2))) ))/( p(0)* std::sqrt(1+ std::pow(p(1)/p(0),2))  ) -
													( std::cos(Lambda*Omega)*std::cos((1+Lambda)*std::atan(p(1)/p(0))) + (1+Lambda)*std::sin((1+lambda)*std::atan(p(1)/p(0))) -
															std::cos(Lambda*Omega)*std::cos((1-Lambda)*std::atan(p(1)/p(0)))  -  (1-Lambda)*p(1)*std::sin((1-Lambda)* std::atan(p(1)/p(0))))/(std::pow(p(0),2)*std::pow( (1+p(1)/p(0)) , (3/2) )  ) +
															( -( std::cos(Lambda*Omega)* (1+Lambda)* std::sin((1+Lambda)*std::atan(p(1)/p(0))) )/(p(0)*(1+std::pow(p(1)/p(0),2)) )  +
																	...
																	((1-Lambda)* std::cos(Lambda*Omega)* std::sin( (1-Lambda)* std::atan(p(1)/p(0)))   )/(p(0)*(1+std::pow(p(1)/p(0),2)) ) -
																	( std::pow((1-Lambda),2)* std::cos((1-Lambda)* std::atan(p(1)/p(0))))/(p(0)*(1+std::pow(p(1)/p(0),2)) )   )/( std::sqrt(1+ std::pow(p(1)/p(0),2)) );
	...
	(1-Lambda)* std::sin((1-Lambda)*std::atan(p(1)/p(0))) ))/(std::pow(p(0),2)* std::sqrt(1+std::pow(p(1)/p(0) ,2))) +
							(std::pow(p(1),3)*( std::cos(Lambda*Omega)*std::cos((1+Lambda)* std::atan(p(1)/p(0)))+ (1+Lambda)*std::sin((1+Lambda)*std::atan(p(1)/p(0))) +
									...
									(p(1)*( ((1+Lambda)*p(1)* std::cos(Lambda*Omega)* std::sin((1+Lambda)* std::atan(p(1)/p(0))) )/(std::pow(p(0),2)* (1+ std::pow( p(1)/p(0) ,2))) -
											...
											-( (1-Lambda)*p(1)*std::cos(Lambda*Omega)* std::sin((1-Lambda)*std::atan2(p(1)/p(0))) )/(std::pow(p(0),2)* (1+ std::pow( p(1)/p(0) ,2)))
	...
	gradients[1][1]= 1/(std::pow(r,2))* ( std::pow(r,Lambda) *Lambda* p(1)* ( (p(1)*( std::cos(Lambda*Omega)* std::cos((1+Lambda)*std::atan(p(1)/p(0)))+(1+Lambda)*std::sin((1+Lambda)*std::atan(p(1)/p(0)))-std::cos(Lambda*Omega)*std::cos((1-Lambda)*std::atan(p(1)/p(0)))-(1-Lambda)*std::sin((1-Lambda)*std::atan(p(1)/p(0)))  ) )/(p(0)*std::sqrt(1+std::pow(p(1)/p(0),2)))
			...
			+std::pow(r,Lambda)* ( (std::cos(Lambda*Omega)* std::cos((1+Lambda)*std::atan(p(1)/p(0))) + (1+Lambda)*std::sin((1+Lambda)*std::atan(p(1)/p(0))) -std::cos(Lambda*Omega)* std::cos((1-Lambda)*std::atan(p(1)/p(0))) - (1-Lambda)*std::sin((1-Lambda)*std::atan2(p(1)/p(0))) )/(p(0)*std::sqrt(1+ std::pow(p(1)/p(0) ,2))  )
					- ( std::pow(p(1),2)* (  std::cos(Lambda*Omega)* std::Cos((1+Lambda)*std::atan(p(1)/p(0)))   + (1+Lambda)*std::sin((1+Lambda) *std::atan(p(1)/p(0))) - std::cos(Lambda*Omega)*std::cos((1-Lambda)*std::atan(p(1)/p(0)))  - (1-Lambda)*std::sin((1-Lambda)*std::atan(p(1)/p(0)))  ))/( std::pow(p(0),3)* std::pow( (1+ std::pow(p(1)/p(0),2) )), 3/2 )
					+ (p(1)* (-( (1+Lambda)*std::cos(Lambda*Omega)*std::sin((1+Lambda)*std::atan(p(1)/p(0))))/(p(0)*(std::sqrt(1+std::pow(p(1)/p(0),2))))
							...
							+( (std::cos(Lambda*Omega)* std::sin((1+Lambda)*std::atan(p(1)/p(0))) )/(1+Lambda) - std::cos((1+Lambda)*std::atan(p(1)/p(0)))  - ( std::cos(Lambda*Omega)* std::sin((1-Lambda)*std::atan(p(1)/p(0))) )/(1-Lambda)  + std::cos((1-Lambda)*std::atan(p(1)/p(0))))/(std::pow(p(0),2)* std::pow( (1+std::pow(p(1)/p(0),2)) , 3/2) )
